...
- RTMP协议由两部分组成，一个是RTMP Chunk Stream协议，一个是RTMP Message协议，其中后者是前者的上层协议。
- RTMP Chunk Stream协议的基本数据单位为Chunk，RTMP Message协议的基本数据单位是message。
- RTMP Chunk Stream可以承载任何数据，不一定是RTMP Message协议。
- RTMP Message协议可以使用其他下层协议传输。
